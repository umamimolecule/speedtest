...
export function App() {
  ...
  const formatRate = (megabytesPerSecond?: number): React.ReactNode => {
    const value =
      megabytesPerSecond === null || megabytesPerSecond === undefined
        ? '--'
        : Math.round(megabytesPerSecond);
    return (
      <div className="flex flex-row gap-4 items-baseline">
        <div className="text-7xl text-white proportional-nums">{value}</div>
        <div className="text-4xl text-neutral-400 font-light">Mb/s</div>
      </div>
    );
  };
  ...
  const renderProgressBar = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-blue-500 self-start"
  ...
  return (
    <div className="app">
      <div className="flex flex-col gap-10 items-center justify-center min-w-[350px]">
        <div className="text-2xl text-neutral-500 mb-4 font-thin">
          Network speed test
        </div>
        {runningTest ? (
      ...
      <div className="">
        <a target="_blank" href="https://github.com/umamimolecule/speedtest">
          <img
            className="absolute bottom-16 right-16 border-solid border-2 border-transparent hover:border-neutral-600 p-1 rounded-full w-12 h-12"
